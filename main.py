...
async def collect_user_data(state: FinancState) -> FinancState:
...
async def detect_intent(state: FinancState) -> FinancState:
    user_input=state["user_input"]
    short_term_memory=state.get("short_term_memory",{})
...
async def get_stock_indo(state:FinancState)->FinancState:
    user_input=state["user_input"]
    short_term_memory=state.get("short_term_memory",{})
    user_profile=state.get("user_profile",{})
    ...
    if not re.match(r'^[A-Z]{1,5}$', stock_symbol) or stock_symbol == 'UNKNOWN':
        ...
        try:
            ...
            if "Time Series (Daily)" in data:
                ...
                stock_data = time_series[latest_data] 

                close_price=stock_data["4. close"]
                message = f"The latest closing price for {stock_symbol} is ${close_price} (as of {latest_data})."
...
async def track_epenses(state:FinancState)->FinancState:
    user_input=state["user_input"]
    short_term_memory=state.get("short_term_memory",{})
    user_profile=state.get("user_profile",{})
...
async def budget_summary(state:FinancState)->FinancState:
    user_profile=state.get("user_profile",{})
    prompt = (
...
async def provide_advice(state:FinancState)->FinancState:
    user_input=state["user_input"]
    user_profile=state.get("user_profile",{})
    long_term_memory=state.get("long_term_memory",{})
...
async def human_in_loop(state:FinancState)->FinancState:
    user_input=state["user_input"]
    prompt = (
